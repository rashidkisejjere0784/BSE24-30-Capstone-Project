...
const { app, server, closeDatabase } = require('./server');  // Import app, server, and closeDatabase
...
describe('Server running test', () => {
  ...
  afterAll(async () => {
    if (testServer) {
      await new Promise((resolve) => testServer.close(resolve));  // Close the server after all tests
    }
